    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="/change_photo" method="post" style="justify-content: center;" enctype="multipart/form-data">
                {{ csrf_field() }}
                <p class="p-1 text-center">Veuillez importer votre demi photo 4x4 qui sera présente sur votre carte</p>
            ...
            @isset($ecole)
                <div class="col-sm-2" style="text-align: center;">
                    <form action="/imprimer" method="POST">
                        ...
                        <button type="submit" class="btn btn-outline-primary"><i class="ri-file-word-2-line"></i></button>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="text-align: center;">
                                <a href="#" onclick="showDeleteCandidatModal('{{$u->code_user}}-{{$filiere}}')" class="btn-outline-danger rounded p-1"><i class='bx bx-x-circle'></i> </a>
                                <a href="/update_info/{{$u->code_user}}-{{$filiere}}"  class="btn-outline-success rounded p-1"><i class='bx bx-pencil'></i> </a>
                            @if ($u->photo_user == null)
                                    <a href="#"  onclick="showUpdatePhotoModal('{{$u->code_user}}')" class="btn-outline-primary rounded p-1"><i class='bx bxs-image'></i> </a>
                            @else
                                    <a href="#"  onclick="showUpdatePhotoModal('{{$u->code_user}}')" class="btn-outline-success rounded p-1"><i class='bx bxs-image'></i> </a>
                            @endif
                            </td>
                            <td> {{$loop->index+1}}  </td>
                            <td> {{$u->code_user}}  </td>
                            ...
                            <td style="text-align: center;">
                                <a href="#" onclick="showDeleteCandidatModal('{{$u->code_user}}-{{$filiere}}')" class="btn-outline-danger rounded p-1"><i class='bx bx-x-circle'></i> </a>
                                <a href="/updatne_info/{{$u->code_user}}-{{$filiere}}"  class="btn-outline-success rounded p-1"><i class='bx bx-pencil'></i> </a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>

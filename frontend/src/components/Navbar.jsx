    ...
    <div>
      <div className='bg-teal-400 w-full p-2 flex items-center justify-center'>
        <h1 className='text-white text-center'>
          Inscription en cours {(candidat === 'null' || candidat === null) ?  <Link to={'/candidate'}>Veuillez vous inscrire </Link>:""}
        </h1>
      </div>
        ...
        <div className='hidden md:block'>
          {
            hasToken !== null ? (<div className='flex gap-2'>
              ...
              <Link to={'/logout'} className='p-2 text-white bg-red-600 rounded-md' onClick={logoutAction}>Déconnexion</Link>
            ...
            <li>
              {
                hasToken !== null ? (<div className='flex gap-2'>
                  <Link to={'/success'} className='p-2 text-white bg-slate-400 rounded-md'>Mon Compte</Link>
                  <Link to={'/logout'} className='p-2 text-white bg-red-600 rounded-md' onClick={logoutAction}>Déconnexion</Link>
                </div>) : <Link to={'/login'} className='p-2 text-white bg-teal-400 rounded-md' onClick={toggleMobileMenu}>Connexion</Link>
              }
            </li>
          </ul>
        </div>,

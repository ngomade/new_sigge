    ...
    if (notEmpty(formData)) {
      ...
      try {
        ...
        if (response.status == 500) {
          ...
          //onClose();
          return toast.error(error, {autoClose: 5000});
        }
        if (response.status === 200) {
          setLoadingState(false);
          onClose();
          const data = await response.json()
          const {access_token,compte} = data
    ...
    <div className="flex flex-col gap-5 lg:flex-row" id="form_pay">
        ...
        <form onSubmit={onSubmit}>
          ...
          <div className="relative flex flex-col gap-3 mb-3">
            ...
            <p className="text-red-500">
              Reçu scanné en PDF, ne dépassant pas 2 Mo
            </p>
          </div>
          <div className="w-full flex items-center justify-center">
            <button type="submit" className="w-1/2 p-2 text-white bg-teal-600 rounded-md">
              Soumettre
            </button>
          </div>
        </form>
      ...
      <div id="preview" className="flex-1">
        ...
        <div className="text-lg text-center md:text-justify shadow-inner shadow-green-800 rounded p-6 animate-bounce">
          <p>Apres la création de votre compte, vous recevrez par mail vos informations de connexion. veuillez vérifier vos spams également. </p>
        </div>
      </div>
    </div>
  );

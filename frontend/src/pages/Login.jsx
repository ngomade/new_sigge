...
function LoginPage() {
    ...
    try {
      loginAPI(formData).then(async(res)=>{
        if(res.status===200){
          ...
          const {access_token,token_type,user, user_type} = data
  ...
  return (
    <div className='w-full h-[90vh] flex justify-center  items-center'>
      <div className="w-full max-w-4xl p-8 bg-white shadow-lg rounded-md flex-col md:flex-row flex justify-between items-center">
        ...
        <h2 className='text-5xl font-bold text-teal-500'>
          ...
          <form onSubmit={onSubmit} className='space-y-6'>
            ...
            <div className='flex flex-col gap-3 mb-3 text-center italic'>
              <a href={'/pwd-recover'} className='text-sky-800'>Cliquez ici si vous avez oublié votre mot de passe</a>
            </div>
